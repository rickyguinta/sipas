      ...
      <!-- AKHIR FLASH DATA -->
          <div class="title-bar">
            <h1 class="title-bar-title">
              <span class="d-ib">DATA NOTA DINAS MASUK</span>
            </h1>
          </div>
          <hr>
            ...
            <div class="col-xs-12">
              <div class="card">
                <div class="card-header">
                  ...
                  <strong>Daftar Nota Dinas Masuk</strong>
                </div>
                <div class="card-body">
                <table id="demo-datatables-colreorder-2" class="table table-hover table-striped table-bordered dataTable" cellspacing="0" width="100%">
                    ...
                    <?php $no=1; foreach($data as $d){ 
                        $id_notdis = $d['id_surat_notdis'];  
                        $get_persetujuan = $this->db->query("SELECT * FROM persetujuan_notdis WHERE id_surat_notdis='$id_notdis' AND is_read=1 ORDER BY id_persetujuan_notdis DESC LIMIT 1")->row_array(); 
                        if($get_persetujuan['id_penerima'] == $d['kepada_notdis']){
                    ?>
